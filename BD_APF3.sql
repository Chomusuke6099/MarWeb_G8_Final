...
CREATE TABLE Cliente (
    ...
    CONSTRAINT fk_cliente_zona
        FOREIGN KEY (ZonaID)
        REFERENCES Zona_Delivery(ZonaID)
        ON DELETE SET NULL
);
...
INSERT INTO Zona_Delivery (Nombre, PalabrasClave, Costo, TiempoEstimado) VALUES 
...
('Zona Sur', 'La Victoria sur, Ciudad Jardín, Los Parques, Urb. Santa María, Av. Salaverry', 5.50, '35-45 minutos'),
...
BEGIN
    ...
    FROM Zona_Delivery
    WHERE p_direccion LIKE CONCAT('%', PalabrasClave, '%')
       OR PalabrasClave LIKE CONCAT('%', p_direccion, '%')
    LIMIT 1;
END //
...
BEGIN
    SELECT p.ProductoID, p.Nombre, p.Descripcion, p.Precio, p.Imagen, c.Nombre AS Categoria
    FROM Producto p
